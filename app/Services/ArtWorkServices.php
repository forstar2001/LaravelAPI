...
class ArtWorkServices
{
    ...
    */
    public function uploadPost($post) 
    {
        ...
        $this->resizeImageToMaxWidth(base_path().'/public/images/posts/' . $file_name,200,$new_image_url);
...
//                                ->where('user_id','!=',$user_id)
        ...
        if(count($posts)>0)
        {
            ...
            foreach ($posts as $post)
            {
                ...
                $post_details[$a]['thumb_image']      = $post->thumb_image;
    ...
    */
    public function postDetails($post)
    {
        ...
        if(count($post_details)>0)//check post exist
        {
            ...
            $result['thumb_image']      = $post_details->thumb_image;
    ...
    */
    public function userFavourites($details)
    {
        ...
        $posts   = Favourite::join('active_posts','active_posts.post_id','=','favourites.post_id')
                            ...
                            ->select('active_posts.post_id','active_posts.user_id','active_posts.post_image','active_posts.post_name','active_posts.post_thumb_image','likes','comments','active_posts.template_id','active_posts.created_at','active_posts.username','active_posts.sub_only','active_posts.template_name','active_posts.image_png_url','active_posts.thumb_image')
