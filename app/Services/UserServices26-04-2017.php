    ...
    */
    public function userProfile($user) 
    {
        ...
        $posts      = ActivePosts::where('user_id',$user_id)
                                   ->select('post_id','post_image','post_name','post_thumb_image','likes','comments','template_id','template_name','image_png_url','sub_only','username','created_at')
    ...
    */
    public function otherUserDetails($details)
    {
        ...
        if(count($user_details)>0)
        {
            ...
            $posts        = ActivePosts::where('user_id',$user_id)
                                        ->select('post_id','user_id','post_image','post_name','post_thumb_image','comments','likes','template_id','template_name','image_png_url','sub_only','username','created_at')
                                        ->orderBy('post_id','DESC') 
                                        ->get();
            $post_details = [];
            //check user liked the post
